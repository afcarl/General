...
select game.id,goal.player from game inner join goal
...
select g.mdate, e.teamname from game g join eteam e on (g.team1=e.id) where e.coach = 'Fernando Santos'
...
select matchid,mdate,count(gtime)
from game join goal on (id=matchid) where ((team1 = "GER" or team2 = "GER") and teamid="GER")
group by matchid,mdate;
